...
const BusinessView = ({
  colorCode, userName, authUser, vCardFile, cardClickedHandler, clickedNo, links, socialLinksOrder, workPhone, workFax, homeFax, note,
  ...
  activeForm, connectionSettings, redirect, isEmbedForm, defaultLinksToTheme, address, marker, connectionTags, settings, email, homePhone,
}) => {
  ...
  return (
    <Box>
      {redirect && isTheLoggedinUser && (
        <Box style={{ maxWidth: 550, margin: '16px auto 0 auto' }}>
          <Alert
      ...
      {(!passwordProtected || isTheLoggedinUser) && (
        <>
          ...
          {links && <CustomLinks links={links} colorCode={colorCode} countClicks={countClicks} profileType="basic" />}
