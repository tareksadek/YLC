...
import AddCustomLinkDialog from './AddCustomLinkDialog'
...
const ProfileLinks = ({
  links, loading, error, onAddLink, onUpdateLink, onToggleLink,
  onRedirectLink, onDeleteCustomLink, onSetNotification, socialLinksOrder, onSort, onCustomSort, redirect,
  onRedirectCardLink, defaultLinksToTheme, onToggleDefaultLinksToTheme, socialLinksSorted,
  customLinksSorted, onUpdateSocialLinks, onSortSocialLinks, onSortCustomLinks, onAddCustomLink, onDeactivateLink, profileLinks, disableActions,
}) => {
  ...
  const validPlatformUrl = (url, platform) => {
    ...
    const linkProtocolExists = url.substring(0, 8) === 'https://' || url.substring(0, 7) === 'http://' || url.substring(0, 8) === 'Https://' || url.substring(0, 7) === 'Http://'
  ...
  const linkChangeHandler = (e, platform, validationRules) => {
    ...
    if (linkValue !== '' || linkValue !== 'http://' || linkValue !== 'https://' || linkValue !== 'Http://' || linkValue !== 'Https://') {
      onUpdateLink(platform, linkValue, validationRules)
    }
  }
  ...
  const linkBlurHandler = (e, platform, validationRules) => {
    ...
    onUpdateLink(platform, inputValue, validationRules)
  }
  ...
  const handleRedirect = (e, redirectLink) => {
    onRedirectLink(e.target.checked, redirectLink)
    onRedirectCardLink(e.target.checked, redirectLink)
  }
      ...
      <Box className={`${layoutClasses.panel}`}>
        ...
        <Box className={classes.linksContainer}>
          ...
          <SocialLinksList
            ...
            onToggleDefaultLinksToTheme={onToggleDefaultLinksToTheme}
            loading={loading}
...
const mapDispatchToProps = dispatch => ({
  onAddLink: link => dispatch(actions.addLink(link)),
  onUpdateLink: (platform, link, validationRules) => dispatch(actions.updateLink(platform, link, validationRules)),
  onToggleLink: platform => dispatch(actions.toggleLink(platform)),
  onDeactivateLink: platform => dispatch(actions.deactivateLink(platform)),
...
const mapStateToProps = state => ({
  ...
  profileLinks: state.cards.links,
...
ProfileLinks.propTypes = {
  ...
  onDeactivateLink: PropTypes.func.isRequired,
